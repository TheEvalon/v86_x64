...
/// IA-32e/PAE NX (bit 63). Not a physical-address bit.
...
/// Execute-disable: data hits may succeed; instruction fetch must miss or #PF.
...
pub unsafe fn readable_or_pagefault(addr: i32, size: i32) -> OrPageFault<()> {
    ...
    let user = *cpl == 3;
    translate_span(addr, size, false, user)
}
...
pub unsafe fn writable_or_pagefault_cpl(other_cpl: u8, addr: i32, size: i32) -> OrPageFault<()> {
    ...
    translate_span(addr, size, true, user)
}

unsafe fn translate_span(addr: i32, size: i32, for_writing: bool, user: bool) -> OrPageFault<()> {
    translate_address(addr, for_writing, user, false, true, false)?;
    let start = virt64_from_i32(addr);
    let last = start.wrapping_add(size as u64 - 1);
    if start & !0xFFF != last & !0xFFF {
        let old = *pending_linear64;
        *pending_linear64 = last;
        let r = translate_address(last as i32, for_writing, user, false, true, false);
        *pending_linear64 = old;
        r?;
    }
    Ok(())
}
...
pub fn translate_address_exec(address: i32) -> OrPageFault<u32> {
    unsafe { translate_address(address, false, *cpl == 3, false, true, true) }
}
pub unsafe fn translate_address_exec_jit(address: i32) -> OrPageFault<u32> {
...
fn tlb_entry_ok(entry: i32, user: bool, for_writing: bool, for_execute: bool) -> bool {
    entry
        & (TLB_VALID
            | if user { TLB_NO_USER } else { 0 }
            | if for_writing { TLB_READONLY } else { 0 }
            | if for_execute { TLB_NO_EXEC } else { 0 })
        == TLB_VALID
}
...
pub unsafe fn translate_address(
    ...
    let mut entry = tlb_data[(address as u32 >> 12) as usize];
    if !tlb_entry_ok(entry, user, for_writing, for_execute) {
        entry = do_page_walk(address, for_writing, user, jit, side_effects, for_execute)?.get();
    }
...
pub unsafe fn translate_address_write_and_can_skip_dirty(address: i32) -> OrPageFault<(u32, bool)> {
    ...
    let mut entry = tlb_data[(address as u32 >> 12) as usize];
    let user = *cpl == 3;
    if !tlb_entry_ok(entry, user, true, false) {
...
// - 1GB pages via PDPTE.PS (physical base bits 51:30; 32-bit phys => 1GB-aligned)
//
...
unsafe fn walk_ia32e_to_pd(
    ...
    *allow_exec &= !pte64_is_nx(pdpte);
    if pdpte as i32 & PAGE_TABLE_PSE_MASK != 0 {
        // 1GB page: bits 29:13 are reserved (PAT is bit 12). Physical base is
        // bits 51:30; with 32-bit phys that is bits 31:30.
        if pdpte as u64 & 0x3FFF_E000 != 0 {
            if side_effects {
        ...
        // 4K TLB granule inside the 1GB page: phys bits 31:30 from the PDPTE,
        // bits 29:12 from the linear address (bits 11:0 stay in the offset).
        let phys = (pdpte as u32 & 0xC000_0000) | (addr as u32 & 0x3FFF_F000);
        let global = pdpte as i32 & PAGE_TABLE_GLOBAL_MASK == PAGE_TABLE_GLOBAL_MASK;
        return Ok(Ia32eWalk::Page1G { phys, global });
    }
    if side_effects {
    ...
    Ok(Ia32eWalk::Pd(
        (pdpte as u32 & 0xFFFFF000) + ((((addr >> 21) & 0x1FF) as u32) << 3),
    ))
}
    ...
    if cr0 & CR0_PG == 0 {
        ...
        let (page_dir_addr, page_dir_entry) = if lma {
            ...
            match walk_ia32e_to_pd(
                ...
                Ia32eWalk::Page1G { phys, global: g } => {
                    page1g = Some((phys, g));
                    allow_write_upper = allow_write;
                    (0, PAGE_TABLE_PRESENT_MASK)
                },
            ...
            if pdpt_entry as i32 & PAGE_TABLE_PRESENT_MASK == 0 {
                if side_effects {
                    trigger_pagefault(addr, false, for_writing, user, jit, for_execute);
                }
                return Err(());
            }
            allow_exec &= !pte64_is_nx(pdpt_entry as i64);
...
unsafe fn do_page_walk_high(
    ...
    match walk_ia32e_to_pd(
        ...
        Ia32eWalk::Page1G { phys, global } => {
            let is_in_mapped_range = memory::in_mapped_range(phys);
            let info_bits = TLB_VALID
                | if for_writing { 0 } else { TLB_READONLY }
                | if allow_user { 0 } else { TLB_NO_USER }
                | if is_in_mapped_range { TLB_IN_MAPPED_RANGE } else { 0 }
                ...
                | if allow_exec { 0 } else { TLB_NO_EXEC };
            let page = (addr as u32 >> 12) as i32;
            let tlb_entry = (phys + memory::mem8 as u32) as i32 ^ page << 12 | info_bits as i32;
            if side_effects {
...
pub unsafe fn translate_address64(
    ...
    if tlb_hash_tag[idx] != tag || !tlb_entry_ok(entry, user, for_writing, for_execute) {
        entry =
            do_page_walk_high(address, for_writing, user, jit, side_effects, for_execute)?.get();
    }
...
pub unsafe fn trigger_pagefault(
    ...
    trigger_pagefault_virt(
        virt64_from_i32(addr),
        present,
        write,
        user,
        jit,
...
pub unsafe fn trigger_pagefault_virt(
    addr: u64,
    present: bool,
    write: bool,
    user: bool,
    ...
    if config::LOG_PAGE_FAULTS {
        dbg_log!(
            "page fault{} w={} u={} p={} fetch={} eip={:x} cr2={:x}",
            if jit { "jit" } else { "" },
            write as i32,
            user as i32,
            present as i32,
            instruction_fetch as i32,
            *previous_ip,
            addr
        );
        dbg_trace();
    ...
    *cr2_64 = addr;
    ...
    let error_code =
        (instruction_fetch as i32) << 4 | (user as i32) << 2 | (write as i32) << 1 | present as i32;
    if jit {
        jit_exit_reason = JitExitReason::CpuException {
...
pub unsafe fn efer_nxe() -> bool { *efer & EFER_NXE != 0 }
...
unsafe fn phys_of_linear_rip(eip: u64) -> OrPageFault<u32> {
    ...
    let phys = if eip <= 0xFFFF_FFFF {
        translate_address_exec(eip as i32)?
    }
    else {
